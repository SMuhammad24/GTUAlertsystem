...
class GTUWebHandler(SimpleHTTPRequestHandler):
    ...
    Serves both dynamic REST endpoints and static web frontend assets.
    """
    ...
    def do_GET(self):
        ...
        # 3. API: Health Check (Used by Render/Railway uptime monitors)
        elif path == '/api/health':
            self._send_json({
                'status': 'healthy',
                'uptime': 'active',
                'database_exists': Path(Config.DB_PATH).exists()
            })
            return
    ...
    def do_POST(self):
            ...
            try:
                ...
                for c in circulars:
                    if not db.is_processed(c['id']):
                        db.save_circular(c['id'], c['title'], c['link'], c['date'], c.get('category', 'General'))
                        new_count += 1
        ...
        # Student Auth: Send OTP via real Email SMTP
            ...
            try:
                ...
                self._send_json({
                    ...
                    # If email failed (e.g. SMTP not configured yet), provide code for seamless fallback
                    'otp_code': otp if not email_sent else None,
                    'message': f"Verification code sent to {email}!" if email_sent else f"OTP generated ({status_msg})"
                })
            except Exception as e:
                self._send_json({'success': False, 'error': str(e)}, status=500)
            return
...
def run_server(port: int = 8080, host: str = "0.0.0.0"):
    ...
    print("=" * 60)
    print(f"🌐 GTU Alerts Web & API Server is LIVE!")
...
if __name__ == '__main__':
    # Render and cloud platforms supply dynamic port via $PORT
    port_env = os.getenv('PORT') or os.getenv('WEB_PORT') or '8080'
    port = int(port_env)
